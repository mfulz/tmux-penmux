...
cd_session() {
  ...
  if [ -e ".pmses" ]; then
    ...
    # "$HOME/.tmux/plugins/tmux-penmux/modules/auxiliary/misc/session/session.sh" -a load -m "$HOME/.tmux/plugins/tmux-penmux/modules/auxiliary/misc/session.xml" -c "$HOME/.tmux/plugins/tmux-penmux/scripts" -f "$session_file"
  fi
}
